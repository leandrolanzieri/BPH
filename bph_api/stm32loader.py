...
class CommandInterface(object):
    def __init__(self, customDTR = None, customRTS = None):
        if customDTR or customRTS:
            wpi.wiringPiSetupGpio()

        self.customDTR = customDTR
        self.customRTS = customRTS

        if self.customDTR:
            wpi.pinMode(self.customDTR, wpi.GPIO.OUTPUT)

        if self.customRTS:
            wpi.pinMode(self.customRTS, wpi.GPIO.OUTPUT)
    ...
    def open(self, aport='/dev/tty.usbserial-FTD3TMCH', abaudrate=115200) :
        ...
        print('Closing and opening serial port')
        self.sp.close()
        self.sp.open()
    ...
    def setRTS(self, state):
        if self.customRTS:
            wpi.digitalWrite(self.customRTS, state)
        else:
            self.sp.setRTS(state)

    def setDTR(self, state):
        if self.customDTR:
            wpi.digitalWrite(self.customDTR, state)
        else:
...
def usage():
    ...
    -D          Set custom pin for DTR
    -R          Set custom pin for RTS
...
# http://www.python.org/doc/2.5.2/lib/module-getopt.html
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hqVewvrXudSp:D:R:b:a:c:s:l:k")
    ...
    for o, a in opts:
        if o == '-V':
            ...
            conf['customDTR'] = eval(a)
        elif o == '-R':
            conf['customRTS'] = eval(a)
        else:
    ...
    cmd = CommandInterface(customDTR = conf.get('customDTR'),
            customRTS = conf.get('customRTS'))
